ui <- fluidPage(
  ...
  googlemap_autocomplete_search_ui("teste", width = "100%", key = Sys.getenv("GCP_TOKEN_MAPS")),
  leaflet::leafletOutput("map", width = "auto", height = "auto")
)

server <- function(input, output, session) {
  address <- googlemap_autocomplete_search_server("teste", key = Sys.getenv("GCP_TOKEN_MAPS"))
