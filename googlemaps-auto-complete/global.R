...
googlemap_autocomplete_search_ui <- function(id, label = "Type an address", width = NULL, placeholder = NULL, key = NULL, ...) {
  ...
  shiny::tagList(
    htmltools::div(
      ...
      shiny::textInput(inputId = button, label = label, width = width, placeholder = placeholder),
      htmltools::HTML(
        stringr::str_interp(
          stringr::str_c("
                        <script>
  ...
  shiny::moduleServer(
    ...
    function(input, output, session) {
      ...
      address <- reactive({
        ...
        result <- googleway::google_geocode(address = my_address(), key = key)
        return(result)
      })
